/*
	Implementation part of binaryTreesVisitor visiting a rotated version of a binary tree.
...
#include<cassert>
  ...
  void binaryTreesVisitor::visit(int numLeaves)  {
    assert(numLeaves<=maxNodes);
		n=numLeaves;
	...
	void binaryTreesVisitor::firstlevel()
  {
  ...
  void binaryTreesVisitor::upperlevel()
  {
    ...
    {
			int nnodesnoleaf=(nperl-numLeavesLeft)/2;
			if(nnodesnoleaf<0)nnodesnoleaf=0;
			for(;nnodesnoleaf <= nperl/2;++nnodesnoleaf)
        pairsUpper(0,nperl-nnodesnoleaf*2,nnodesnoleaf,(1<<nperl)-1); 
    }
  }
  
  void binaryTreesVisitor::pairs1st(int minid,int nnodes)
  {
    if(nnodes==0){
      ++level;
  ...
  void binaryTreesVisitor::pairsUpper(
    int minid,
