...
private:
	...
	{
		tset t;
		//for every leaf, a bit saying if its filled
		int f;

		union children {
		//(of type Node or Leaf)
	...
	ND root;
	//memory for the nodes stored
	std::vector<ND> memfornodes;
	
	int depth;

	//the tensor sets in the leaf bins
	std::vector<tset>  tsets;
...
public:
	...
	*/
	TsTree(const std::vector<long double>&pts , long double boxwidth=0.01,int method=1);
	~TsTree(void);
};
#endif
